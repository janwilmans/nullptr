...
The assertion can in some cases hang a continuous integration system because it [pops up a message dialog][2] by default, however, as far as I know this is applies to MSVC only.
...
Here you can see that checking a reference for null can have interesting side effects. The check is completely optimized out, fortunately clang is nice enough to emit emit a warning when this happens.
...
So in C++ we have smartpointers right? Raw-pointers? so passé! NO. Let me be clear: raw pointers are great, always have been, likely always will be. However, **owning** raw pointers, those are or should be a thing of the past. An owning raw pointers looks like this:
